...
public class LoginWeb {
    ...
    // 通过Get方式获取HTTP服务器数据
    public static ArrayList<HashMap<String,String>> login(String account, String password) {
        ...
        try {
            // 用户名 密码
            // URL 地址
            String path = "http://" + IP + "/SYCserver/Login";
            path = path + "?account=" + account + "&password=" + password;
            Log.i("LoginWeb","NO1. "+account+" "+password);
            ...
            if (conn.getResponseCode() == 200) {
                Log.i("LoginWeb","NO2. "+account+" "+password);
